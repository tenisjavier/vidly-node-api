...
// @desc    return movies from an especific genreId or null 404 not found
// @access  public
router.get("/genre/:id", validateId, async (req, res) => {
    const result = await movieController.getMovieByGenre(req.params.id);
    if (!result) return res.status(404).send("Movie not found.");
    res.send(result);
});
...
router.put("/:id", [auth, validateId], async (req, res) => {
    const result = await movieController.updateMovieById(
        req.params.id,
        req.body
    );
    if (!result) return res.status(404).send("Movie not found.");
